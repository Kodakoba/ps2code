...
#include <kernel.h> 
#include <stdlib.h>
#include <malloc.h> //standard c libraries for drawing
#include <tamtypes.h>
#include <math3d.h>

#include <packet.h> //networking?

#include <dma_tags.h>
#include <gif_tags.h> //different ps2 libs
#include <gs_psm.h>
...
#include <draw.h> //draw and draw recursive (3D)
#include <draw3d.h>

#include "mesh_data.c" //our mesh data file. I do wish to learn more about how this is read and used.
//is zlib allowed here to just open .obj files live or no

VECTOR object_position = {0.00f, 0.50f, 0.50f, 1.00f}; //why does VECTOR look so scary
VECTOR object_rotation = {0.00f, 0.00f, 0.00f, 0.00f};

VECTOR camera_rotation = {0.00f, 0.00f, 0.00f, 0.00f};
VECTOR camera_position = {0.00f, 0.00f, 100.00f, 1.00f}; //camera lol

void init_gs(framebuffer_t *frame, zbuffer_t *z)
{

	// Define a 32-bit 640x512 framebuffer.
    // well originally. NOW I AM THE ONE WHO MAKES THE RULES MUAHAHAH
	frame->width = 640; //720i is broken lmfao
	frame->height = 512;
	frame->mask = 0;
	frame->psm = GS_PSM_32;
...
//now we do the next procedure after init the GS
...
int render(framebuffer_t *frame, zbuffer_t *z)
{
	...
	for (;;)
	{
		...
		// Spin the cube a bit.
		object_rotation[0] += 0.000f; //while (object_rotation[0] > 3.14f) { object_rotation[0] -= 6.28f; }
		object_rotation[1] += 0.100f; //while (object_rotation[1] > 3.14f) { object_rotation[1] -= 6.28f; }
